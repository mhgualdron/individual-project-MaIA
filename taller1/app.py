...
# Load data from csv
def load_data():
    path = os.path.join(os.getcwd(), 'taller1/datos_energia.csv')
    data_energia = pd.read_csv(path)
    data_energia['time'] = pd.to_datetime(data_energia['time'])
    data_energia.set_index('time', inplace=True)
    return data_energia
